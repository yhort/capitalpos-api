...
public class InfrastructureDependencyInjectionTests
{
    ...
    public void Add_capital_pos_infrastructure_registra_repositorios_ef()
    {
        ...
        Assert.Contains(services, descriptor =>
            ...
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(ICategoriaRepository) &&
            descriptor.ImplementationType == typeof(EfCategoriaRepository) &&
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IMarcaRepository) &&
        ...
        Assert.Contains(services, descriptor =>
            ...
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IReglaPrecioMayoristaRepository) &&
            descriptor.ImplementationType == typeof(EfReglaPrecioMayoristaRepository) &&
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            ...
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(ICompraRepository) &&
            descriptor.ImplementationType == typeof(EfCompraRepository) &&
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            ...
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(IPedidoDigitalRepository) &&
            descriptor.ImplementationType == typeof(EfPedidoDigitalRepository) &&
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
        ...
        Assert.Contains(services, descriptor =>
            ...
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
            descriptor.ServiceType == typeof(ISesionCajaRepository) &&
            descriptor.ImplementationType == typeof(EfSesionCajaRepository) &&
            descriptor.Lifetime == ServiceLifetime.Scoped);
        Assert.Contains(services, descriptor =>
